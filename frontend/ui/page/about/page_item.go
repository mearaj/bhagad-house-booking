...
func (i *pageItem) layoutContent(gtx Gtx) Dim {
	...
	if i.btnSetCurrentIdentity.Clicked() {
		i.shouldCloseMenuItems = false
		if !i.settingAsPrimaryBooking {
			i.settingAsPrimaryBooking = true
			...
			go func() {
				...
				//i.Manager.Service().SetAsCurrentBooking(i.Customer.PublicKey)
				i.settingAsPrimaryBooking = false
