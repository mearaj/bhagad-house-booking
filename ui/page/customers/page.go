...
type page struct {
	...
	*view.ModalContent
	SelectionMode            bool
	fetchingCustomersCh      chan []service.Customer
...
func New(manager Manager) Page {
	...
	p := page{
		...
		contactsView:             []*pageItem{},
		menuIcon:                 iconMenu,
		fetchingCustomersCh:      make(chan []service.Customer, 10),
...
func (p *page) drawDeleteCustomersModal(gtx Gtx) Dim {
	...
	gtx.Constraints.Max.Y = int(float32(gtx.Constraints.Max.Y) * 0.85)
	if p.btnYes.Clicked() {
		contacts := make([]service.Customer, 0)
...
func (p *page) OnDatabaseChange(event service.Event) {
	...
	case service.BookingChangedEventData, service.BookingsChangedEventData:
		p.fetchCustomersCount()
		if len(p.contactsView) == 0 {
			p.fetchCustomers(0, defaultListSize)
		} else {
	...
	case service.CustomersChangeEventData:
		p.fetchCustomersCount()
		if len(p.contactsView) == 0 {
			p.fetchCustomers(0, defaultListSize)
		} else {
